...
export default async ({ aquarius, analytics }) => {
  aquarius.onCommand(/^fizzbuzz (?<input>[\d]+)$/i, async (message, { groups }) => {
    log(`Checking ${groups.input}`);
    aquarius.loading.start(message.channel);

    try {
      const response = await fetch(`https://fozzbizz.herokuapp.com/fizzbuzz?num=${groups.input}`);
      ...
      log(e);
      message.channel.send('Unable to fizzbuzz. Ping @IanMitchel1 on twitter with your complaints');
    }
