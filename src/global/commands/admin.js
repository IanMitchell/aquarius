...
async function getGuildTarget(aquarius, message) {
  ...
  if (guilds.size === 0) {
    ...
    try {
      const msg = await aquarius.directMessages.prompt(message.author, "You're an admin in several guilds - what is the ID of the one you want to check?");
...
export default async ({ aquarius, analytics }) => {
  ...
  aquarius.onDirectMessage(/settings list (?<command>.+)/i, async (message, { groups }) => {
    log('Setting list');
  ...
  aquarius.onDirectMessage(/set (?<command>.+) (?<setting>.+) (?<value>.+)/i, async (message, { groups }) => {
    ...
    if (!aquarius.guildManager.get(guild).enabledCommands.has(command)) {
      ...
      channel.send(`It doesn't look like ${command} is enabled on your server!`);
      return;
    }

    if (!aquarius.commandConfigs.get(command).keys().includes(setting)) {
      log(`Invalid setting name: ${setting}`);
      channel.send(`It doesn't look like ${command} has a setting called "${setting}"`);
      return;
    }
  ...
  aquarius.onDirectMessage(/unset (?<command>.+) (?<setting>.+)/i, async (message, { groups }) => {
    ...
    if (!aquarius.guildManager.get(guild).enabledCommands.has(groups.command)) {
      log(`${command} isn't enabled on server`);
      channel.send(`It doesn't look like ${groups.command} is enabled on your server!`);
      return;
    }
  ...
  aquarius.onCommand(/commands add (?<commands>.+)/i, async (message, { groups }) => {
    if (aquarius.permissions.isGuildAdmin(message.guild, message.author)) {
      log(`Add ${groups.commands} in ${message.guild.name} by ${message.author.username}`);
      ...
      if (addedCommands.length > 0) {
        response += `Added the ${humanize(addedCommands)} ${pluralize('command', addedCommands.length)}. \n\n`;
      }

      if (commandsWithSettings.length > 0) {
        response += `You can configure the ${humanize(commandsWithSettings)} ${pluralize('command', commandsWithSettings.length)} - direct message me \`settings list <name>\` with a command name to learn more.\n\n`;
      }

      if (unknownCommands.length > 0) {
        response += `I don't recognize the ${humanize(unknownCommands)} ${pluralize('command', unknownCommands.length)}.`;
      }
  ...
  aquarius.onCommand(/commands remove (?<commands>.+)/i, async (message, { groups }) => {
    if (aquarius.permissions.isGuildAdmin(message.guild, message.author)) {
      log(`Remove ${groups.commands} in ${message.guild.name} by ${message.author.username}`);
      ...
      if (removedCommands.length > 0) {
        response += `Removed the ${humanize(removedCommands)} ${pluralize('command', removedCommands.length)}. `;
      }
      ...
      if (globalCommands.length > 0) {
        response += `The ${humanize(globalCommands)} ${pluralize('command is', globalCommands.length, 'commands are')} considered 'Global' - Global commands are core to Aquarius and can't be disabled.`;
      }
