...
export default async ({ aquarius, analytics }) => {
  aquarius.onCommand(/^nick(?:name)? (?<nickname>.*)/i, async (message, { groups }) => {
    if (message.member.hasPermission(Permissions.FLAGS.MANAGE_NICKNAMES)) {
      const check = aquarius.permissions.check(message.guild, ...info.permissions);
      ...
      try {
        log(`Updating Nickname to '${groups.nickname}' in ${message.guild.name}`);
        await member.setNickname(groups.nickname);
        message.channel.send('Nickname changed!');
        analytics.trackUsage('nickname', message);
      } catch (error) {
        message.channel.send("I couldn't update my nickname - please make sure it's valid!");
      }
    } else {
      message.channel.send("You don't have permission to change my nickname!");
    }
  });
};
