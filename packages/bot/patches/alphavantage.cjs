...
(function(global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined'
    ? (module.exports = factory(require('cross-fetch')))
    : typeof define === 'function' && define.amd
    ? define(['cross-fetch'], factory)
    : ((global = global || self),
      (global.alphavantage = factory(global.fetch)));
})(this, function(fetch) {
  'use strict';
  ...
  var Forex = config => {
    ...
    return {
      rate: (from_currency, to_currency) =>
        util.fn('CURRENCY_EXCHANGE_RATE')({ from_currency, to_currency }),
    };
  };
  ...
  var Technical = config => {
     ...
     */
    const HT_LIKE = fn => (symbol, interval, series_type) =>
      util.fn(fn)({ symbol, interval, series_type });
    ...
    return {
      ...
      sar: (symbol, interval, acceleration, maximum) =>
        util.fn('SAR')({ symbol, interval, acceleration, maximum }),
      trange: SMA_LIKE('TRANGE'),
      atr: SMA_LIKE('ATR'),
      natr: SMA_LIKE('NATR'),
      ad: SMA_LIKE('AD'),
      adosc: (symbol, interval, fastperiod, slowperiod) =>
        util.fn('ADOSC')({ symbol, interval, fastperiod, slowperiod }),
      obv: SMA_LIKE('OBV'),
  ...
  var index = (config = {}) => {
    ...
    if (config.key === undefined) {
      throw new Error("Missing Alpha Vantage 'key' in config");
    }
